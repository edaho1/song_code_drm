 ...
 * gcc song.c aes_encrypt.c aes_decrypt.c utils.c dec.c -g -O0 -o dec
...
int main(int argc, char **argv)
{
    ...
    while ((read = fread(&buffer16,sizeof(char)*16,1,inFile))> 0)
    {
        if( read != 7463438/16){
            ...
            assert(decrypt_song(master_key, tmp, p));
            p+=16;
        }
    }

    // writing output to file
    write = fwrite(decode, sizeof(char)*7463438,1, outFile);
